...
namespace WMAgility.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            if (!await _roleManager.RoleExistsAsync(WebConstants.AdminRole))
            {
                ...
                await _roleManager.CreateAsync(new IdentityRole(WebConstants.HandlerRole));
            }
            ReturnUrl = returnUrl;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        //an admin role had logged in and they try to create a new user
                        await _userManager.AddToRoleAsync(user, WebConstants.AdminRole);
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, WebConstants.HandlerRole);
                    }
